    ...
    questions: [
      {
        ...
        answer: 1,
        whyWrong: {
          0: 'O(n) is a linear scan — checking each element one at a time. Binary search is exponentially faster because it halves the search range every step.',
        ...
        explanation:
          'Correct. Each step halves the search range, so the number of steps is ⌈log₂ n⌉. Doubling n adds one more comparison, not double the work.',
      },
      {
        ...
        explanation:
          'Correct. Inner body runs n × n = n² times. This is the classic quadratic pattern: fine at n=100, brutal at n=1,000,000.',
      },
      {
        ...
        explanation:
          'Correct. Dynamic arrays double capacity on resize, so n appends cost O(n) total → O(1) per append on average. Individual resizes are O(n) but rare.',
      },
    ],
  },
    ...
    questions: [
      {
        ...
        explanation:
          'Correct. Large η pushes θ past the minimum each step. The loss landscape becomes a bouncing ball. Common signal: loss goes up after one epoch.',
      },
      {
        ...
        answer: 1,
        whyWrong: {
          0: 'Underfitting shows BOTH train and val loss as high — the model can\'t even fit the training data. Here train loss is dropping fine.',
        ...
        explanation:
          'Correct. The gap between train and val loss is the overfitting signature. Stop early, add regularization, or get more data.',
      },
      {
        ...
        explanation:
          'Correct. Predicting all-negative scores 95% accuracy on a 95/5 dataset while catching zero positives. Use precision/recall/F1 for imbalanced data.',
      },
    ],
  },

  // ─────────────────────────────────────────────────────────────────────────
  // MLOps · Retrieval-Augmented Generation
  // ─────────────────────────────────────────────────────────────────────────
  'llm-rag': {
    ...
    questions: [
      {
        ...
        explanation:
          'Correct. Cosine similarity = 1 when vectors are aligned, 0 when orthogonal, −1 when opposite. Magnitude is normalized out — only direction matters.',
      },
      {
        ...
        answer: 1,
        whyWrong: {
          0: 'More retrieved chunks LOWER precision — you\'re reaching deeper into the ranked list where similarity scores are weaker. Hallucinations can actually rise as noise drowns the signal.',
        ...
        explanation:
          'Correct. Bigger k = more relevant context but also more noise and a fatter token bill. Tuning k is a precision/recall/cost tradeoff, not a free lunch.',
      },
      {
        ...
        answer: 1,
        whyWrong: {
          0: 'Bigger chunks make the problem WORSE — even more irrelevant text gets dragged in with the one relevant sentence.',
        ...
        explanation:
          'Correct. Smaller chunks let the retriever pinpoint the relevant sentence instead of dragging in a whole page. 256–1024 tokens is a common sweet spot.',
      },
    ],
  },

  // ─── Additional banks (workflow-generated) ──────────────────────────────
    ...
    "questions": [
      {
        ...
        "answer": 1,
        "whyWrong": {
          "0": "That's mean ABSOLUTE error (MAE), which uses |residual| rather than residual². MAE penalizes a 10-unit miss as 10× a 1-unit miss; MSE penalizes it as 100×.",
      ...
      },
      {
        ...
        "answer": 1,
        "whyWrong": {
          "0": "Moving along +∇L is gradient ASCENT — you'd climb the loss surface. That's what you do when maximizing a likelihood, not minimizing a loss.",
      ...
      },
      {
        ...
        "options": [
          ...
          "η is too large — each step overshoots the minimum and bounces across the bowl",
          "The model is overfitting the training set",
          "The gradient has vanished to zero"
        ],
        "answer": 1,
        "whyWrong": {
          "0": "Too-small η produces slow but MONOTONICALLY decreasing loss — flat and boring, not bouncing. Oscillation is the opposite failure mode.",
        ...
        "whyCorrect": "For a convex quadratic loss (which linear regression is), there's a stability ceiling: η must be below 2/λ_max where λ_max is the top eigenvalue of the Hessian XᵀX. Above that, each step overshoots and amplifies the error — the iterates diverge. Oscillating loss is the canonical signal.",
      ...
      },
      {
        ...
        "answer": 1,
        "whyWrong": {
          "0": "X⁻¹ only exists when X is SQUARE and invertible — i.e., exactly as many samples as features with no redundancy. Real datasets have n ≫ d (tall X), so X⁻¹ isn't even defined.",
      ...
      },
      {
        ...
        "options": [
          ...
          "XᵀX becomes nearly singular; the inverse blows up and weights become huge and unstable",
          "The bias term b grows without bound",
          "Training loss goes to infinity"
        ],
        "answer": 1,
        "whyWrong": {
          "0": "OLS does NOT handle near-duplicate columns gracefully. Predictions can still look fine on the training set, but the individual weights become wildly unstable and useless for interpretation.",
        ...
        "whyCorrect": "When two columns are nearly identical, det(XᵀX) → 0 and the matrix is near-singular. Its inverse has huge eigenvalues, so tiny noise in y gets amplified into massive, often opposite-signed weights on the correlated features. Predictions stay reasonable; coefficients become meaningless.",
  ...
  "ml-core-logistic-regression": {
    ...
    "questions": [
      ...
      },
      {
        ...
        "answer": 1,
        "whyWrong": {
          "0": "The centroid (data mean) has no special relationship to where σ = 0.5. The boundary is determined by the learned w and b, not by where the data clusters.",
      ...
      },
      {
        ...
        "answer": 1,
        "whyWrong": {
          "0": "MSE is perfectly well-defined for p ∈ (0,1) — it's just (y − p)². The problem isn't definedness; it's the shape of the loss surface.",
      ...
      },
      {
        ...
        "answer": 1,
        "whyWrong": {
          "0": "Squared-distance minimization is what SVMs (hinge loss + margin) and MSE do. Logistic regression doesn't care about geometric margin directly — it cares about probability calibration via likelihood.",
    ...
    "questions": [
      {
        ...
        "whyWrong": {
          ...
          "3": "Rotation is one special case (when A is a rotation matrix), not the general rule. Most matrices also scale and shear; reducing A·x to 'rotation by some angle' hides shear, scaling, and reflection."
        },
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "3": "Rank-nullity holds for ANY matrix (rectangular included): rank + nullity = number of columns. The question is well-posed; this answer is a dodge."
        },
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "3": "Eigenvalues are properties of the covariance MATRIX, independent of sample count (once you've estimated C). A component isn't 'used' by a fixed number of samples."
        },
    ...
    "questions": [
      {
        ...
        "whyWrong": {
          ...
          "3": "You confused the value of the derivative with the value of the function. dL/dθ is the slope OF L at θ, not L itself."
        },
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "3": "The steepest-ascent direction is the FULL gradient ∇L = (∂L/∂θ₁, ∂L/∂θ₂). A single partial is just one component of that vector, not the whole direction."
        },
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "3": "The gradient is perpendicular to LEVEL SETS (contours of constant L), not to the surface itself. Confusing level-set normals with surface normals is a common geometry slip."
        },
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "3": "You inverted the derivatives. dg/dy and dx/dg are the wrong objects entirely — there's no guarantee g is invertible, and even when it is, this combination doesn't equal dy/dx."
        },
      ...
      },
      {
        ...
        "options": [
          ...
          "∇L points uphill; subtracting it moves θ downhill, lowering the loss",
        ...
        "whyWrong": {
          ...
          "3": "Adding the gradient would do gradient ASCENT — climbing the loss surface, making the model worse every step. The sign is mathematically required, not stylistic."
        },
        "whyCorrect": "∇L is the direction of steepest INCREASE in loss, so −∇L is steepest decrease. Stepping by −η·∇L moves θ to a (locally) lower-loss point, with η controlling step size. Flip the sign and you'd be running gradient ascent — maximizing loss, which is what you'd do to find adversarial examples, not to train.",
    ...
    "questions": [
      {
        ...
        "whyWrong": {
          ...
          "3": "You multiplied prior × sensitivity but skipped Bayes' denominator P(+). Without normalizing by the total probability of a positive test, the answer isn't a valid probability."
        },
      ...
      },
      {
        ...
        "options": [
          ...
          "Products of many small probabilities underflow to 0; sums of logs are numerically stable and turn the product into an additive loss",
        ...
        "whyWrong": {
          ...
          "3": "Any strictly monotonic transform preserves the argmax. We pick log specifically because it converts products to sums and tames floating-point underflow, not because it's mathematically privileged."
        },
        "whyCorrect": "A dataset of n points gives a likelihood ∏ᵢ p(xᵢ|θ). For n = 1000 and typical p ≈ 0.001, that product is 10⁻³⁰⁰⁰ — underflows to zero in float64. Taking log turns it into ∑ᵢ log p(xᵢ|θ), which stays in a sane numeric range AND gives per-example gradients that decompose nicely across mini-batches.",
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "3": "Variance is a property of the posterior, not a selection rule. Picking the lower-variance estimator confuses 'how certain am I?' with 'what's my best guess under this loss function?'"
        },
      ...
      },
      {
        ...
        "options": [
          ...
          "Centered near 1 — the likelihood overwhelms the prior as data grows",
        ...
        "whyWrong": {
          ...
          "3": "Disagreement between prior and likelihood is exactly the situation Bayes is designed for. The posterior is always well-defined as long as p(data|θ) · p(θ) is integrable; the data simply tells you the prior was wrong."
        },
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "2": "Retraining isn't required — you can adjust posteriors analytically by reweighting with the new prior ratio. This is called prior correction or threshold shifting and avoids re-fitting the whole model.",
    ...
    "questions": [
      {
        ...
        "whyWrong": {
          ...
          "2": "Scaling the dot product by a scalar doesn't change Q or K's lengths — those vectors are untouched. You'd need to L2-normalize Q and K row-wise (cosine attention) for that, which is a different design.",
      ...
      },
      {
        ...
        "options": [
          ...
          "d_k = 64 per head; multiple heads attend to different subspaces in parallel",
        ...
        "whyWrong": {
          ...
          "2": "Heads don't concatenate inputs — they SPLIT them. Each head sees the full sequence but only a d_model/h slice of the channel dimension. The concatenation happens on the OUTPUT side, after each head's attended values.",
      ...
      },
      {
        ...
        "options": [
          ...
          "Self-attention is permutation-equivariant — without PE, 'dog bites man' = 'man bites dog'",
        ...
        "whyWrong": {
          ...
          "2": "Variance normalization is what LayerNorm does. Sinusoidal PE adds a deterministic vector to the embedding — it doesn't standardize the input distribution.",
      ...
      },
      {
        ...
        "whyWrong": {
          ...
          "2": "Cubic would require three sequence-length dimensions in the activation. The score matrix is n × n, so two — exactly quadratic.",
    ...
    "questions": [
      {
        ...
        "whyWrong": {
          ...
          "2": "Swaps the two metrics. The picker likely put TP/actual on top for precision and TP/predicted on top for recall — the definitions reversed.",
      ...
      },
      {
        ...
        "options": [
          ...
          "The model is poorly calibrated — it's overconfident",
        ...
        "whyCorrect": "A well-calibrated model with output p̂ = 0.9 should be right ~90% of the time on that bucket — here it's right only 60%. That's classic overconfidence: useful rankings, untrustworthy probabilities. Cost-sensitive decisions (\"is this email worth quarantining?\") rely on p̂ matching reality.",
    ...
    "questions": [
      ...
      },
      {
        ...
        "options": [
          ...
          "Sigmoid saturates at both tails so its gradient vanishes; ReLU has gradient 1 for x > 0, so deep stacks don't choke on tiny gradients",
        ...
        "whyCorrect": "Sigmoid's derivative peaks at 0.25 (at x=0) and decays to ~0 for |x| > 4. Stack ten sigmoid layers and the backprop gradient gets multiplied by ten values each ≤ 0.25 — you're shipping ≤ 0.25¹⁰ ≈ 10⁻⁶ to early layers, which never learn. ReLU's derivative is exactly 1 for x > 0, so gradients pass through unchanged.",
      ...
      },
      {
        ...
        "options": [
          ...
          "Every layer is forced to pass through the origin: if X = 0 then Z = 0, regardless of W, so the network can't shift activations to wherever the data lives",
    ...
    "questions": [
      ...
      },
      {
        ...
        "options": [
          ...
          "Vanishing gradients — products of small σ' values collapse toward zero",
        ...
        "whyCorrect": "Sigmoid's derivative peaks at 0.25 and is much smaller in the saturating tails. Backprop multiplies one σ' per layer, so in a 10-layer net the early gradient is at best 0.25¹⁰ ≈ 10⁻⁶ of the output gradient. The signal literally dies before it reaches W₁, so those weights stop updating.",
      ...
      },
      {
        ...
        "options": [
          ...
          "Variance rises ~16×; usually lower the LR or use gradient accumulation",
